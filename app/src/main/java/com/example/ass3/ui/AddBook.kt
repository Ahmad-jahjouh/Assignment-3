...
//        }
        ...
        add_but.setOnClickListener {
            ...
            val Bookname = add_name.text.toString()
            val Author = add_Author.text.toString()
            val year= yearAdd.text.toString()
            val Price = add_Price.text.toString()
            val rate = add_ratingBar.rating
    ...
    private  fun sendNotification(notification: PushNotification)= CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            if (response.isSuccessful){
                Toast.makeText(this@AddBook, "aaaa", Toast.LENGTH_SHORT).show()
            }else{
